...
from pytorch.model.cifar10_CNN import ConvNet_cifar10
from pytorch.model.FashionMNIST_CNN import FashionCNN
from pytorch.model.MNIST_CNN import MNIST_CNN
from codecarbon import OfflineEmissionsTracker

logging.basicConfig(filename='../pytorch/experiment.log',
                    level=logging.INFO)


def Load_dataset_and_model(modelPATH, dataset):  # TODO doing custom datasets part
...
def Experiment(
    ...
    # TODO enlever les print dans le script final
    print(f'Num GPUs available: {torch.cuda.device_count()}')
    print(f'Num CPUs available: {os.cpu_count()}')
    ...
    if loss == "CrossEntropyLoss":  # TODO ajouter les autres loss dans Pytorch
        criterion = nn.CrossEntropyLoss()

    if optimizer == "SGD":  # TODO ajouter les autres optimizer les plus connus
    ...
    logging.info(f'pytorch for dataset {dataset}, with the model saved her "{modelPATH}"')
    logging.info(f'learning rate = {learningRate}')

    print(f'pytorch for dataset {dataset}, with the model saved her "{modelPATH}"')
...
if __name__ == "__main__":
    ...
    dataset = sys.argv[1]
    measure_power_secs = float(sys.argv[2])
    batch_sizes = sys.argv[3]
    modelPATH = sys.argv[4]
    nbrEpoch = sys.argv[5]
    learningRate = sys.argv[6]
    optimizer = sys.argv[7]
    loss = sys.argv[8]
    emission_output_directory = sys.argv[9]
    emission_output_file = sys.argv[10]
    final_result_PATH = sys.argv[11]

    results, code_carbon_output_path = Experiment(dataset="MNIST",
                                                  measurePowerSecs=600000,
                                                  batchSizes=1,
                                                  ...
                                                  emission_output_file="emissions_mnist.csv"
                                                  )
